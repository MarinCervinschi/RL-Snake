...
class PyGameRenderer:
    def __init__(self, grid_size: int = 10, cell_size: int = 30, speed: float = 0.1):
        self.grid_size = grid_size
        self.cell_size = cell_size
        self.speed = speed

        self.grid_pixel_width = grid_size * cell_size
        self.grid_pixel_height = grid_size * cell_size
        ...
        pygame.display.set_caption("RL Snake - DQN Agent")
    ...
    def _draw_ui(self, score: int, record: int):
        ui_y = self.window_height - 45

        score_text = self.font.render(f"Score: {score}", True, self.colors["TEXT"])
        self.display.blit(score_text, (10, ui_y))

        record_text = self.font.render(f"Record: {record}", True, self.colors["TEXT"])
        text_rect = record_text.get_rect()
        text_rect.topright = (self.window_width - 10, ui_y)
